...
# ============ 检测后端配置（cone_detector 并入 NRT_WS 后新增） ============
# 后端选择：'trt'（TensorRT）→ 'onnx'（ONNX Runtime）→ 'yolov5'（PyTorch 回退）
# 可用环境变量覆盖：NRT_DETECTOR / NRT_TRT_ENGINE / NRT_ONNX_MODEL
NRT_DETECTOR = os.environ.get('NRT_DETECTOR', 'trt')          # 默认 TensorRT
NRT_BATCH_MERGE = (os.environ.get('NRT_BATCH_MERGE', '0') == '1')  # 双图合并 batch=2 推理（需 engine 按 --batch 2 构建）
CONF_THRESH = 0.45        # 置信度阈值（YOLO11n 更准，取原 0.5 与 cone_detector 0.25 的折中）
NMS_IOU_THRESH = 0.45     # NMS IOU 阈值
TRT_INPUT_SIZE = 640      # 模型输入尺寸

# 注入 cone_detector 共享推理模块路径（跨包 import，不依赖 catkin 包安装）
_CAMERA_DIR = os.path.dirname(os.path.abspath(__file__))
_SRC_DIR = os.path.dirname(_CAMERA_DIR)
if os.path.isdir(os.path.join(_SRC_DIR, 'cone_detector')):
    sys.path.insert(0, _SRC_DIR)

CONE_DETECTOR_DIR = os.path.join(_SRC_DIR, 'cone_detector')
TRT_ENGINE_PATH = os.environ.get(
    'NRT_TRT_ENGINE',
    os.path.join(CONE_DETECTOR_DIR, 'models', 'yolo26n_cone.engine')
)
ONNX_MODEL_PATH = os.environ.get(
    'NRT_ONNX_MODEL',
    os.path.join(CONE_DETECTOR_DIR, 'models', 'yolo26n_cone.onnx')
)
# ========================================================================

# ---------------------- 配置参数 ----------------------
weight_path = "/home/nvidia/yolov5/weights1/best.pt"  # YOLO权重路径（PyTorch 回退）
...
def load_model(weights):
    ...
    后端优先级（自动回退，可用环境变量 NRT_DETECTOR 强制指定）:
      1. TensorRT  .engine  —— cone_detector 的 YOLO11n 加速引擎（默认）
      2. ONNX Runtime .onnx —— 备选
      3. YOLOv5 PyTorch .pt —— 原后端回退
    """
    try:
        from cone_detector.inference import ConeDetector
    except ImportError as e:
        print(f"[加载] 无法导入 cone_detector.inference: {e}，回退 YOLOv5 PyTorch")
        from models.experimental import attempt_load
        model = attempt_load(weights, device=device)
        model.engine_type = 'yolov5'
        print(f"模型加载成功（设备：{device}，后端：YOLOv5 PyTorch）")
        return model
    ...
    try:
        model = ConeDetector(
            ...
            backend=NRT_DETECTOR,
            conf_thresh=CONF_THRESH,
            iou_thresh=NMS_IOU_THRESH,
            input_size=TRT_INPUT_SIZE,
        )
        print(f"模型加载成功（后端：{model.engine_type}）")
        return model
    except Exception as e:
        print(f"模型加载失败：{e}")
        raise
...
def detect_objects(model, img):
    """检测图像中的锥桶并返回中心点坐标（统一后端接口）

    后端（TensorRT / ONNX / YOLOv5 PyTorch）均通过统一 detect() 契约
    返回相同 schema：{label, center, bbox, confidence}，主循环无需改动。
    """
...
def main():
    ...
    try:
        ...
        while running and not rospy.is_shutdown():
            ...
            # 检测锥桶（双图合并 batch=2 推理，需 NRT_BATCH_MERGE=1 且 TRT 动态 batch engine）
            if NRT_BATCH_MERGE and getattr(model, 'engine_type', None) == 'tensorrt':
                try:
                    batch_res = model.detect_batch([current_left, current_right])
                    left_dets, right_dets = batch_res[0], batch_res[1]
                except Exception as e:
                    print(f"[batch] 合并推理失败，回退顺序推理: {e}")
