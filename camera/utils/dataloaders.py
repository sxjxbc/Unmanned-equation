...
class LoadStreams:
    """YOLOv5 视频流加载器，支持 YouTube、IP 相机、GStreamer"""

    def __init__(self, sources="file.streams", img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        torch.backends.cudnn.benchmark = True  # 固定尺寸推理加速
        self.mode = "stream"
        self.img_size = img_size
        self.stride = stride
        ...
        self.auto = auto
        self.transforms = transforms
        self.sources = Path(sources).read_text().rsplit() if os.path.isfile(sources) else [sources]
        n = len(self.sources)
        #self.sources = [x if x.startswith(("gst://", "gst-launch")) else clean_str(x) for x in sources]
        self.imgs, self.fps, self.frames, self.threads = [None] * n, [0] * n, [0] * n, [None] * n

        for i, s in enumerate(self.sources):  # 遍历每个流
            st = f"{i + 1}/{n}: {s}... "
            if urlparse(s).hostname in ("www.youtube.com", "youtube.com", "youtu.be"):  # YouTube
                check_requirements(("pafy", "youtube_dl==2020.12.2"))
                import pafy
                s = pafy.new(s).getbest(preftype="mp4").url
            elif s.startswith("gst://") or s.startswith("gst-launch"):  # ========= GStreamer =========
                from utils.dataloaders import LoadGstreamer
                dataset = LoadGstreamer(s, img_size=img_size, stride=stride, auto=auto)
                return  # 直接返回，不再走 OpenCV
            s = eval(s) if s.isnumeric() else s  # 数字索引
            if s == 0:
                assert not is_colab(), "Colab 不支持本地摄像头"
                assert not is_kaggle(), "Kaggle 不支持本地摄像头"

            cap = cv2.VideoCapture(s)
            print("[DEBUG] original source ->", s)
    ...
    def update(self, i, cap, stream):
        """线程：持续读帧"""
        n, f = 0, self.frames[i]
        while self.threads[i].is_alive() and n < (f - 1):
            n += 1
            cap.grab()  # 只抓不解码，快
            if n % self.vid_stride == 0:  # 按 stride 解码
    ...
    def __next__(self):
        self.count += 1
        if not all(self.threads[i].is_alive() for i in range(len(self.sources))):
            raise StopIteration
        img0 = self.imgs.copy()
        if self.transforms:
            img = np.stack([self.transforms(cv2.cvtColor(x, cv2.COLOR_BGR2RGB)) for x in img0])
        else:
            img = np.stack([letterbox(x, self.img_size, stride=self.stride, auto=self.auto)[0] for x in img0])
            img = img[..., ::-1].transpose(0, 3, 1, 2)  # BGR→RGB, HWC→CHW
            img = np.ascontiguousarray(img)
        return self.sources, img, img0, [None] * len(self.sources)
...
def autosplit(path=DATASETS_DIR / "coco128/images", weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    Arguments:
        ...
        weights:         Train, val, test weights1 (list, tuple)
...
class LoadGstreamer:
    def __init__(self, source, img_size=736, stride=32, auto=True):
        import cv2, numpy as np
        from utils.augmentations import letterbox
        self.img_size = img_size
        self.stride   = stride
        self.auto     = auto
        self.pipe     = source.replace("gst://", "")   # 去掉前缀
        self.cap      = cv2.VideoCapture(self.pipe, cv2.CAP_GSTREAMER)
        assert self.cap.isOpened(), f"GStreamer 管道打开失败: {self.pipe}"
    ...
    def __next__(self):
        ...
        img = np.ascontiguousarray(img)
        return self.pipe, img, frame, None   # path, img, original, cap
