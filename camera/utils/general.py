...
def labels_to_class_weights(labels, nc=80):
    """Calculates class weights1 from labels to handle class imbalance in training; input shape: (n, 5)."""
    ...
    # weights1 = np.hstack([gpi * len(labels)  - weights1.sum() * 9, weights1 * 9]) ** 0.5  # prepend gridpoints to start
...
def labels_to_image_weights(labels, nc=80, class_weights=np.ones(80)):
    """Calculates image weights1 from labels using class weights1 for weighted sampling."""
    # Usage: index = random.choices(range(n), weights1=image_weights, k=1)  # weighted image sample
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if merge and (1 < n < 3e3):  # Merge NMS (boxes merged using weighted mean)
            # update boxes as boxes(i,4) = weights1(i,n) * boxes(n,4)
            iou = box_iou(boxes[i], boxes) > iou_thres  # iou matrix
            weights = iou * scores[None]  # box weights1
