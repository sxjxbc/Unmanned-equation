...
def initialize_weights(model):
    """Initializes weights1 of Conv2d, BatchNorm2d, and activations (Hardswish, LeakyReLU, ReLU, ReLU6, SiLU) in the
    model.
    """
...
def smart_optimizer(model, name="Adam", lr=0.001, momentum=0.9, decay=1e-5):
    ...
    Groups are 0) weights1 with decay, 1) weights1 no decay, 2) biases no decay.
    """
    ...
    optimizer.add_param_group({"params": g[1], "weight_decay": 0.0})  # add g1 (BatchNorm2d weights1)
    LOGGER.info(
...
def smart_resume(ckpt, optimizer, ema=None, weights="yolov5s.pt", epochs=300, resume=True):
    ...
    if resume:
        assert start_epoch > 0, (
            ...
            f"Start a new training without --resume, i.e. 'python train.py --weights1 {weights}'"
        )
