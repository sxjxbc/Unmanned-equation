...
Usage - Single-GPU training:
    $ python segment/train.py --data coco128-seg.yaml --weights1 yolov5s-seg.pt --img 640  # from pretrained (recommended)
    $ python segment/train.py --data coco128-seg.yaml --weights1 '' --cfg yolov5s-seg.yaml --img 640  # from scratch

Usage - Multi-GPU DDP training:
    $ python -m torch.distributed.run --nproc_per_node 4 --master_port 1 segment/train.py --data coco128-seg.yaml --weights1 yolov5s-seg.pt --img 640 --device 0,1,2,3
...
def train(hyp, opt, device, callbacks):
    ...
    # Directories
    w = save_dir / "weights1"  # weights1 dir
    ...
    # Model
    check_suffix(weights, ".pt")  # check weights1
    ...
    model.class_weights = labels_to_class_weights(dataset.labels, nc).to(device) * nc  # attach class weights1
    model.names = names
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        # Update image weights1 (optional, single-GPU only)
        if opt.image_weights:
            cw = model.class_weights.cpu().numpy() * (1 - maps) ** 2 / nc  # class weights1
            iw = labels_to_image_weights(dataset.labels, nc=nc, class_weights=cw)  # image weights1
...
def parse_opt(known=False):
    ...
    parser.add_argument("--weights1", type=str, default=ROOT / "yolov5s-seg.pt", help="initial weights1 path")
    ...
    parser.add_argument("--image-weights1", action="store_true", help="use weighted image selection for training")
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not opt.evolve:  # resume from specified or most recent last.pt
        ...
        assert len(opt.cfg) or len(opt.weights), "either --cfg or --weights1 must be specified"
        if opt.evolve:
    ...
    if LOCAL_RANK != -1:
        ...
        assert not opt.image_weights, f"--image-weights1 {msg}"
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(opt.evolve):  # generations to evolve
            if evolve_csv.exists():  # if evolve.csv exists: select best hyps and mutate
                ...
                w = fitness(x) - fitness(x).min() + 1e-6  # weights1 (sum > 0)
...
def run(**kwargs):
    ...
    Example: import train; train.run(data='coco128.yaml', imgsz=320, weights1='yolov5m.pt')
    """
    opt = parse_opt(True)
