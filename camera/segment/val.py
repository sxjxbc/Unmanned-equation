...
Usage:
    ...
    $ python segment/val.py --weights1 yolov5s-seg.pt --data coco.yaml --img 640  # validate COCO-segments

Usage - formats:
    $ python segment/val.py --weights1 yolov5s-seg.pt                 # PyTorch
...
def run(
    ...
    # Dataloader
    if not training:
        if pt and not single_cls:  # check --weights1 are trained on --data
            ncm = model.model.nc
            assert ncm == nc, (
                f"{weights} ({ncm} classes) trained on different --data than what you passed ({nc} "
                f"classes). Pass correct combination of --weights1 and --data that are trained together."
            )
    ...
    if save_json and len(jdict):
        w = Path(weights[0] if isinstance(weights, list) else weights).stem if weights is not None else ""  # weights1
...
def parse_opt():
    """Parses command line arguments for configuring YOLOv5 options like dataset path, weights1, batch size, and
    inference settings.
    """
...
def main(opt):
    ...
    if opt.task in ("train", "val", "test"):  # run normally
        ...
        if opt.task == "speed":  # speed benchmarks
            # python val.py --task speed --data coco.yaml --batch 1 --weights1 yolov5n.pt yolov5s.pt...
            opt.conf_thres, opt.iou_thres, opt.save_json = 0.25, 0.45, False
            for opt.weights in weights:
                run(**vars(opt), plots=False)

        elif opt.task == "study":  # speed vs mAP benchmarks
            # python val.py --task study --data coco.yaml --iou 0.7 --weights1 yolov5n.pt yolov5s.pt...
            for opt.weights in weights:
